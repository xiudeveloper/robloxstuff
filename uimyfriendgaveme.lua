...
local function SIPZBJ_fake_script() -- FakeInternal.MainScript 
	...
	local function BindToggle(key)
		...
		if UserInputService.TouchEnabled then
			...
			ContextActionService:SetTitle(ActionName, "byefron")
...
local function CLAL_fake_script() -- Settings.SettingsScript 
	...
	local MainDirectory = "byefron"
	local SaveDirectory = "byefron\\UI.json"
...
local function MFJYB_fake_script() -- Popular.ScriptHubScript 
	...
	local Directory = "byefron Scripts"
...
local function NYMIRF_fake_script() -- ScriptHub.ScriptHubScript 
	...
	local MainDirectory = "byefron"
	local ScriptsDirectory = "byefron\\Scripts"
	local DirectoryPatternLua = string.format("^%s\\(.+).lua$", ScriptsDirectory) -- "^byefron\\Scripts\\(.+).lua$"
	local DirectoryPatternTxt = string.format("^%s\\(.+).txt$", ScriptsDirectory) -- "^byefron\\Scripts\\(.+).txt$"
	local AssetsDirectory = "byefron\\Assets"
	...
	local function RefreshPersonal()
		if getgenv then
			...
			for i,v in next, listfiles(ScriptsDirectory) do
				...
				if ScriptName:find("byefron\\Scripts") then
					ScriptName = v:gsub(DirectoryPatternTxt, function(a)
						lua = false
						return a
					end)
				end
				if not ScriptName:find("byefron\\Scripts") then
